<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contact Us | BrewHaven Cafe PH</title>
    <link rel="stylesheet" href="../assets/css/includes.css">
    <style>
        /* Contact Page Specific Styles */
        .contact-hero {
            background: linear-gradient(135deg, #FAF9F6 0%, #FFD88F 100%);
            padding: 80px 40px;
            text-align: center;
        }

        .contact-hero h1 {
            font-size: 3.5em;
            color: #3E2723;
            margin-bottom: 20px;
            font-weight: 800;
        }

        .contact-hero .subtitle {
            font-size: 1.3em;
            color: #A0522D;
            max-width: 600px;
            margin: 0 auto;
        }

        .contact-container {
            max-width: 1200px;
            margin: 0 auto;
            padding: 60px 40px;
            background: #FAF9F6;
        }

        .contact-grid {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 60px;
            align-items: start;
        }

        .contact-info {
            background: #FFFFFF;
            padding: 40px;
            border-radius: 20px;
            box-shadow: 0 5px 20px rgba(0,0,0,0.1);
            border: 2px solid #FFD88F;
        }

        .contact-info h2 {
            color: #A0522D;
            font-size: 2.2em;
            margin-bottom: 30px;
            font-weight: 700;
        }

        .info-item {
            display: flex;
            align-items: flex-start;
            gap: 20px;
            margin-bottom: 30px;
            padding: 20px;
            background: #F9F9F9;
            border-radius: 15px;
            transition: all 0.3s ease;
        }

        .info-item:hover {
            background: #FFF9E6;
            transform: translateX(5px);
        }

        .info-icon {
            width: 50px;
            height: 50px;
            background: #FFD88F;
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 1.3em;
            color: #A0522D;
            flex-shrink: 0;
        }

        .info-content h3 {
            color: #3E2723;
            margin-bottom: 5px;
            font-size: 1.3em;
        }

        .info-content p {
            color: #5a3927;
            margin: 0;
            line-height: 1.6;
        }

        .hours-section {
            margin-top: 40px;
            padding: 25px;
            background: #F5F5DC;
            border-radius: 15px;
            border: 1px solid #A0522D;
        }

        .hours-section h3 {
            color: #A0522D;
            margin-bottom: 20px;
            font-size: 1.5em;
        }

        .hours-list {
            list-style: none;
            padding: 0;
        }

        .hours-list li {
            display: flex;
            justify-content: space-between;
            padding: 8px 0;
            color: #3E2723;
            border-bottom: 1px solid #FFD88F;
        }

        .hours-list li:last-child {
            border-bottom: none;
        }

        .contact-form {
            background: #FFFFFF;
            padding: 40px;
            border-radius: 20px;
            box-shadow: 0 5px 20px rgba(0,0,0,0.1);
            border: 2px solid #FFD88F;
        }

        .contact-form h2 {
            color: #A0522D;
            font-size: 2.2em;
            margin-bottom: 30px;
            font-weight: 700;
        }

        .form-group {
            margin-bottom: 25px;
        }

        .form-group label {
            display: block;
            color: #3E2723;
            margin-bottom: 8px;
            font-weight: 600;
        }

        .form-group input,
        .form-group textarea,
        .form-group select {
            width: 100%;
            padding: 15px;
            border: 2px solid #FFD88F;
            border-radius: 10px;
            font-size: 1em;
            background: #FAF9F6;
            transition: all 0.3s ease;
            box-sizing: border-box;
        }

        .form-group input:focus,
        .form-group textarea:focus,
        .form-group select:focus {
            outline: none;
            border-color: #A0522D;
            background: #FFFFFF;
            box-shadow: 0 0 10px rgba(160, 82, 45, 0.2);
        }

        .form-group textarea {
            height: 120px;
            resize: vertical;
        }

        .submit-btn {
            background: linear-gradient(135deg, #D04F4F 0%, #C0392B 100%);
            color: white;
            border: none;
            padding: 18px 35px;
            border-radius: 30px;
            font-size: 1.1em;
            font-weight: bold;
            cursor: pointer;
            transition: all 0.3s ease;
            width: 100%;
            box-shadow: 0 8px 25px rgba(208, 79, 79, 0.3);
        }

        .submit-btn:hover {
            background: linear-gradient(135deg, #FFD88F 0%, #F4D03F 100%);
            color: #A0522D;
            transform: translateY(-3px);
            box-shadow: 0 12px 30px rgba(255, 216, 143, 0.4);
        }

        .location-section {
            margin-top: 60px;
            text-align: center;
            padding: 40px;
            background: #F5F5DC;
            border-radius: 20px;
            border: 2px solid #A0522D;
        }

        .location-section h2 {
            color: #A0522D;
            font-size: 2.2em;
            margin-bottom: 20px;
            font-weight: 700;
        }

        .location-section p {
            color: #5a3927;
            font-size: 1.2em;
            margin-bottom: 10px;
        }

        /* Responsive Design */
        @media (max-width: 968px) {
            .contact-grid {
                grid-template-columns: 1fr;
                gap: 40px;
            }
            
            .contact-hero h1 {
                font-size: 2.8em;
            }
        }

        @media (max-width: 768px) {
            .contact-hero {
                padding: 60px 20px;
            }
            
            .contact-hero h1 {
                font-size: 2.3em;
            }
            
            .contact-container {
                padding: 40px 20px;
            }
            
            .contact-info,
            .contact-form {
                padding: 30px 20px;
            }
            
            .info-item {
                flex-direction: column;
                text-align: center;
                gap: 15px;
            }
        }

        @media (max-width: 480px) {
            .contact-hero h1 {
                font-size: 2em;
            }
            
            .contact-info h2,
            .contact-form h2,
            .location-section h2 {
                font-size: 1.8em;
            }
        }
    </style>
</head>
<body>
    <div class="container">
        <!-- Navigation -->
        <?php include '../includes/nav.php'; ?>

        <!-- Hero Section -->
        <section class="contact-hero">
            <h1>Get In Touch</h1>
            <p class="subtitle">We'd love to hear from you! Reach out with any questions or feedback.</p>
        </section>

        <!-- Contact Container -->
        <section class="contact-container">
            <div class="contact-grid">
                <!-- Contact Information -->
                <div class="contact-info">
                    <h2>Visit Us Today</h2>
                    
                    <div class="info-item">
                        <div class="info-icon">📍</div>
                        <div class="info-content">
                            <h3>Our Location</h3>
                            <p>Cebu City, Cebu<br>Philippines 6000</p>
                        </div>
                    </div>

                    <div class="info-item">
                        <div class="info-icon">📞</div>
                        <div class="info-content">
                            <h3>Call Us</h3>
                            <p>(032) 8123-4567<br>Available 6:00 AM - 10:00 PM</p>
                        </div>
                    </div>

                    <div class="info-item">
                        <div class="info-icon">✉️</div>
                        <div class="info-content">
                            <h3>Email Us</h3>
                            <p>hello@brewhaven.com<br>We respond within 24 hours</p>
                        </div>
                    </div>

                    <div class="hours-section">
                        <h3>Operating Hours</h3>
                        <ul class="hours-list">
                            <li><span>Monday - Friday:</span> <span>6:00 AM - 10:00 PM</span></li>
                            <li><span>Saturday:</span> <span>6:00 AM - 11:00 PM</span></li>
                            <li><span>Sunday:</span> <span>7:00 AM - 9:00 PM</span></li>
                            <li><span>Holidays:</span> <span>7:00 AM - 8:00 PM</span></li>
                        </ul>
                    </div>
                </div>

                <!-- Contact Form -->
                <div class="contact-form">
                    <h2>Give Us Feedback</h2>
                    <form id="contactForm" action="../backend/feedback.php" method="POST">
                        <div class="form-group">
                            <label for="subject">Username</label>
                            <input type="text" id="subject" name="name" placeholder="Username here (optional)">
                        </div>

                        <div class="form-group">
                            <label for="feedback">Your Feedback</label>
                            <textarea id="feedback" name="feedback" required placeholder="Tell us how we can help you..."></textarea>
                        </div>

                        <button type="submit" class="submit-btn" name="submit">Send Message</button>
                    </form>
                </div>
            </div>

            <!-- Simple Location Section -->
            <section class="location-section">
                <h2>Find Our Cafe</h2>
                <p>📍 We're located in the heart of Cebu City</p>
                <p>Come visit us for the best coffee experience in town!</p>
                <p><strong>123 Coffee Street, Cebu City, Cebu 6000</strong></p>
            </section>
        </section>

        <?php include '../includes/feedback_alerts.php'; ?>

        <!-- Footer -->
        <footer class="main-footer">
            <div class="footer-content">
                <div class="footer-section">
                    <h3>BrewHaven Cafe PH</h3>
                    <p>Your Kapihan, Your Haven</p>
                    <p>Experience the perfect blend of Filipino hospitality and artisan coffee.</p>
                </div>
                <div class="footer-section">
                    <h4>Quick Links</h4>
                    <ul>
                        <li><a href="../index.php">Home</a></li>
                        <li><a href="menu.php">Menu</a></li>
                        <li><a href="about.html">About Us</a></li>
                        <li><a href="contact.html">Contact</a></li>
                    </ul>
                </div>
                <div class="footer-section">
                    <h4>Contact Info</h4>
                    <p>📍 Cebu City, Cebu</p>
                    <p>📞 (032) 8123-4567</p>
                    <p>✉️ hello@brewhaven.com</p>
                </div>
            </div>
            <div class="footer-bottom">
                <p>&copy; 2025 BrewHaven Cafe PH. All rights reserved.</p>
            </div>
        </footer>
    </div>

</body>
</html>

